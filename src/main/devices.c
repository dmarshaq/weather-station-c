...
int devices_init(Devices_Info *devices_info) {
    ...
    // Init stream to read bytes from serial ports.
    stream = stream_make(128);
...
void devices_add(char *name, Device_Type type, char *path) {
    ...
    switch(dev_info->devices[dev_info->devices_length].type) {
        ...
        case DEVICE_I2C_SENSOR:
            break;
        case DEVICE_ARDUINO:
            int fd = open(dev_info->devices[dev_info->devices_length].path, O_RDWR);
	    ...
	    if (tcsetattr(fd, TCSANOW, &tty) != 0) {
            ...
            dev_info->devices[dev_info->devices_length].serial_port = fd;
...
ssize_t read_serial_nonblocking(int fd, uint8_t *buffer, size_t maxlen) {
    ssize_t n = read(fd, buffer, maxlen);
    if (n < 0) {
        if (errno == EAGAIN || errno == EWOULDBLOCK) {
		LOG_INFO("No data available skipping serial port: '%d'.", fd);
		return 0;  // No data available.
	}
        perror("Couldn't nonblock read serial port");
        return -1;
    }
...
void read_serial_blocking(int fd, Stream *s) {
	// uint8_t *ptr = buffer;
	uint8_t byte;
	...
	while (1) {
		...
		if (n < 0) {
			...
			perror("Couldn't block read serial port");
			return;
		}
		if (n == 0) {
		...
		stream_write(s, 1, &byte);

		LOG_INFO("Got 1 byte from the serial port.");

		if (byte == 0xAA) {
			LOG_INFO("Got 0xAA byte.");
			start = true;
		}

		if (byte == 0xBB && start) {
			LOG_INFO("Got 0xBB byte ending read.");
			return;
		} 
...
void read_serial(int fd, Data_Point* current_data_point) {
	...
	read_serial_blocking(fd, &stream);


	ssize_t len = stream_tell(&stream);
	char buffer[len];
...
loop_exit:
    ...
    LOG_INFO("Received: %d bytes from the arduino, identified payload of size: %d bytes.", len, end - start);
